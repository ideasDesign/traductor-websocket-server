...
class RobustTranslationServer {
  ...
  setupExpress() {
    this.app.use(cors({
      origin: ['https://traductor.hbcolorado.com', 'http://localhost:8080'],
      credentials: true
    }));
    ...
    this.app.get('/info', (req, res) => {
      ...
      res.json({
        ...
        ...stats,
        listenerUrl: `https://traductor.hbcolorado.com/listen`,
        webSocketUrl: `wss://${process.env.RAILWAY_PUBLIC_DOMAIN || 'localhost:3001'}`
      });
    });
  }
  ...
  createDefaultRoom() {
    if (!this.rooms.has(this.defaultRoom)) {
      const room = new TranslationRoom(this.defaultRoom, 'Sala Principal HB Colorado');
  ...
  startHealthCheck() {
    ...
    setInterval(() => {
      const stats = this.getServerStats();
      console.log(`📊 HB Colorado Server Stats: ${stats.totalClients} clients, ${stats.activeRooms} rooms, ${stats.totalAudioChunks} audio chunks sent`);
    }, 60000);
  }
  ...
  listen(port = 3001) {
    this.server.listen(port, '0.0.0.0', () => {
      ...
      console.log(`
🎵 HB Colorado Translation Server Started
📡 Server: ${domain ? `https://${domain}` : `http://localhost:${port}`}
👥 Frontend: https://traductor.hbcolorado.com
🔗 WebSocket: ${domain ? `wss://${domain}` : `ws://localhost:${port}`}
📊 Health Check: ${domain ? `https://${domain}` : `http://localhost:${port}`}/health
🏠 Rooms API: ${domain ? `https://${domain}` : `http://localhost:${port}`}/rooms
      `);
    });
  ...
  shutdown() {
    console.log('🔄 Shutting down HB Colorado translation server...');
